...
# def randomCrop(img_train, img_label, img_mask, width, height):
def randomCrop(img_train, img_label, width, height):    
    # random.seed(1)

    assert img_train.shape[0] >= height
    assert img_train.shape[1] >= width    
    assert img_label.shape[0] >= height
    assert img_label.shape[1] >= width
    # assert img_mask.shape[0] >= height
    # assert img_mask.shape[1] >= width
    ...
    img_label = img_label[y:y+height, x:x+width]
    # img_mask  = img_mask[y:y+height, x:x+width]

    return img_train, img_label #,img_mask
...
# def process_sub_image(train_name, label_name, mask_name,img_size=96, random_crop=False):
def process_sub_image(train_name, label_name,img_size=96, random_crop=False):    
    ...
    if h < img_size or w < img_size: 
        raise
    if random_crop:
        # return randomCrop(img_train, img_label,img_mask, img_size,img_size)
        return randomCrop(img_train,img_label,img_size,img_size)
    else:
        # return crop_center(img_train, img_label,img_mask, img_size,img_size)
...
def input_setup(args):
    ...
    train_filenames = np.array(glob.glob(os.path.join(args.train_dir, '**', '*.*'), recursive=True))
    #print('ttttt',train_filenames)
    label_filenames = np.array(glob.glob(os.path.join(args.label_dir, '**', '*.*'), recursive=True))
    
    # mask_filenames = np.array(glob.glob(os.path.join(args.mask_dir, '**', '*.*'), recursive=True))

    # val_filenames = np.array(glob.glob(os.path.join(args.val_dir, '**', '*.*'), recursive=True))
    # val_label_filenames = np.array(glob.glob(os.path.join(args.val_label_dir, '**', '*.*'), recursive=True))
    # val_mask_filenames = np.array(glob.glob(os.path.join(args.val_mask_dir, '**', '*.*'), recursive=True)) 

    # eval_indices = np.random.randint(len(train_filenames), size=len(val_filenames))
    # eval_filenames = train_filenames[eval_indices[:112]]#val data len 
    # eval_label_filenames = label_filenames[eval_indices[:112]]
    # eval_mask_filenames = mask_filenames[eval_indices[:112]]

    # print(len(train_filenames))
    #print((train_filenames[eval_indices[:16]]))
    #print((val_filenames))
    ...
    # return train_filenames, label_filenames, mask_filenames,val_filenames, val_label_filenames, val_mask_filenames,eval_filenames, eval_label_filenames, eval_mask_filenames 
    # return train_filenames, label_filenames,val_filenames, val_label_filenames,eval_filenames, eval_label_filenames 
    return train_filenames, label_filenames 
